  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.ovsconfig = {
      ...
      script = ''
        ...
        # Add connections to remote hosts
        ${concatStrings (mapAttrsToList (n: remote: ''
        ovs-vsctl --may-exist add-port obr0 gre-${remote.name} -- set Interface gre-${remote.name} type=ipsec_gre options:remote_ip="${remote.remoteIp}" options:psk=${remote.psk} ofport_request=${toString remote.port}
        '') cfg.remotes)}
        ...
        in ''
          ...
          in ''
            # Handle input flows
            ovs-ofctl -O OpenFlow13 add-flow obr0 "table=0,ip,nw_dst=${inf.subnet},in_port=${toString remote.port},actions=output:${toString inf.port}"
            ovs-ofctl -O OpenFlow13 add-flow obr0 "table=0,arp,nw_dst=${inf.subnet},in_port=${toString remote.port},actions=output:${toString inf.port}"

            # Handle output flows
            ovs-ofctl -O OpenFlow13 add-flow obr0 "table=0,in_port=${toString inf.port},ip,nw_dst=${net.subnet},actions=output:${toString remote.port}"
            ovs-ofctl -O OpenFlow13 add-flow obr0 "table=0,in_port=${toString inf.port},arp,nw_dst=${net.subnet},actions=output:${toString remote.port}"
          '') inf.networks}
        '') cfg.interfaces)}
      '';
    };
